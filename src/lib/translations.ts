...
export const translations = {
  ...
  faq: {
    ...
    a6: t('Definitivamente. MILA está pre-entrenada con miles de casos y observaciones reales de organismos de control. Ayuda a detectar de forma preventiva muchos de los errores típicos (falta de firmas, errores en plazos, justificaciones débiles) antes de que el expediente sea enviado, reduciendo significativamente la probabilidad de recibir observaciones formales.', 'Definitely. MILA is pre-trained with thousands of real cases and observations from control bodies. It helps to preemptively detect many of the typical errors (missing signatures, errors in deadlines, weak justifications) before the file is sent, significantly reducing the probability of receiving formal observations.'),
    ...
    a9: t('Es una pregunta clave. MILA es una herramienta de asistencia y sus sugerencias no son vinculantes. El equipo legal y técnico siempre tiene la última palabra. La plataforma está diseñada para ser un "copiloto" que detecta y sugiere, pero la responsabilidad final de la decisión y del documento recae en el funcionario humano, quien valida o descarta las recomendaciones.', 'This is a key question. MILA is an assistance tool, and its suggestions are not binding. The legal and technical team always has the final say. The platform is designed to be a "co-pilot" that detects and suggests, but the final responsibility for the decision and the document rests with the human official, who validates or dismisses the recommendations.'),
  },
  cta: {
