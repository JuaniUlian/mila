...
export const translations = {
  ...
  faq: {
    ...
    a1: t('No, en absoluto. MILA funciona como un asistente para el equipo legal. Automatiza la primera revisión, detectando errores comunes y repetitivos. Esto libera tiempo valioso para que los abogados se enfoquen en tareas estratégicas de alto nivel que requieren su juicio experto.', 'No, not at all. MILA acts as an assistant for the legal team. It automates the initial review, detecting common and repetitive errors. This frees up valuable time for lawyers to focus on high-level strategic tasks that require their expert judgment.'),
    ...
    a9: t('Es una pregunta clave. MILA es una herramienta de asistencia y sus sugerencias no son vinculantes. El equipo legal y técnico siempre tiene la última palabra. La plataforma está diseñada para ser un "asistente" que detecta y sugiere, pero la decisión final siempre es potestad del criterio experto del funcionario, quien utiliza las sugerencias como un valioso insumo para enriquecer y validar el documento.', 'This is a key question. MILA is an assistance tool, and its suggestions are not binding. The legal and technical team always has the final say. The platform is designed to be an "assistant" that detects and suggests, but the final decision is always at the discretion of the official\'s expert judgment, who uses the suggestions as a valuable input to enrich and validate the document.'),
  },
  cta: {
  ...
  preparePage: {
    ...
    processedIn: t('Procesado en {time}s', 'Processed in {time}s'),
    folderOptions: t('Opciones de carpeta', 'Folder options'),
    renameFolderTitle: t('Renombrar Carpeta', 'Rename Folder'),
    deleteFolderTitle: t('Carpeta Eliminada', 'Folder Deleted'),
    newFolderNameLabel: t('Nuevo nombre para "{folderName}"', 'New name for "{folderName}"'),
    confirmDeleteFolderTitle: t('Confirmar Eliminación de Carpeta', 'Confirm Folder Deletion'),
    confirmDeleteFolderDesc: t('¿Está seguro de que desea eliminar la carpeta {folderName}? Todos los archivos que contiene también serán eliminados. Esta acción no se puede deshacer.', 'Are you sure you want to delete the folder {folderName}? All files within it will also be deleted. This action cannot be undone.'),
    // Regulation actions
  ...
  loadingPage: {
    ...
    status5: t('Finalizando...', 'Finalizing...'),
